...
from api_services.rent_car_company import CarStatus, Car
...
class TestUpdateCar:
    ...
    def add_new_car(self):
        ...
        new_car = Car('TestName', 'TestModel', 'TestType', CarStatus.AVAILABLE.value).get_car_info()
        self.car = json.loads(new_car)
        CarApiService.add_new_car(car=new_car).assert_status_code(200)
        yield
        CarApiService.delete_car(model=self.car['model']).assert_status_code(200)
    ...
    def test_update_car_model(self, add_new_car):
        ...
        assert actual_car_model == new_car_model, f'Incorrect car model after it was updated: actual\n{actual_car_model} but must be\n{new_car_model}'
    ...
    def test_update_car_type(self, add_new_car):
        ...
        assert actual_car_type == new_car_type, f'Incorrect car type after it was updated: actual\n{actual_car_type} but must be\n{new_car_type}'

    def test_update_car_status(self, add_new_car):
        parameter_status = {"status": CarStatus.NOT_AVAILABLE.value}
        update_car_response = CarApiService.update_car(model=self.car["model"], params=parameter_status)\
            .assert_status_code(200)\
            .get_json_message()
        actual_car_status = update_car_response['status']

        assert actual_car_status == CarStatus.NOT_AVAILABLE.value, f'Incorrect car status after it was updated: actual\n{actual_car_status} but must be\n{CarStatus.NOT_AVAILABLE}'
